...
Julian stood out to judges by creating and implementing an LLM Statistics Comparator Web App on his Poster in a **QR Code** that leveraged **Python, Flask, and React to use API Keys to compare  LLM Responses to Exam Statistics Questions on one Landing Page**. Output is compared in terms of its conciseness to the reader and the word count of each response. **Julian created this app and deployed it in the timeline of only one weekend leading up to event.** Web App may or may not run globally due to the expiration of the API Keys (Cost of Web servicing) since the website cost was funded by the school for a temporary period, So I included a YouTube video Demo showcasing the App.
